...
def plot_time_history(dataset, variables=['Te'], upstream_index=2, target_index=-2,
    ...
    # Save the time history plot to the current directory
    time_history_filename = "time_history_plot.png"
    plt.savefig(time_history_filename)
    print(f"Time history plot saved as {time_history_filename}, final time step: {last_time_step} (ms) / {last_time_step*1e-3}(s)")
    plt.close()
...
def plot_profiles_animation(simulation_data, variables=['Te'], data_label=None,
    ...
    """
    num_timesteps = min(40, simulation_data.dims['t'])  # Use last 20 timesteps or fewer
    num_vars = len(variables)
...
def main(directory_path):
    ...
    plot_time_history(ds, variables=['Te','Td+', 'Ne', 'Nd'], track_detachment_front=True)
    
    # Generate profiles plot
    plot_profiles(ds, variables=['Te','Td+', 'Ne', 'Nd'], data_label='Simulation')
